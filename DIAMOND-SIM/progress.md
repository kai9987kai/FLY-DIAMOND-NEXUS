...
- Measured after those fixes: upgraded versus legacy syncytium, +30.65 net score over 20 seeds, 95% interval [+5.00, +65.62]. Rolling each mechanism back individually shows all four contributing. Reported alongside three results that do not flatter the change: a single Antennal Lobe brain now scores 4.4x the full syncytium and beats it on every metric, the spiking arm is inconclusive, and the earlier +95.85 figure was measured on the buggy code.
- 148 Node tests (1 skipped offline), 38 browser checks, JavaScript/HTML checks pass.
